...
public class ArticleDao {
    ...
    public Optional<ArticleData> getById(final Long communityId, final String articleType) {
        ...
        final Map<String, Long> params = Map.of(articleType + "Id", communityId);
        return namedParameterJdbcTemplate.query(sql.replaceAll("\\{\\}", articleType), params, ROW_MAPPER).stream().findAny();
    }

    public Page<ArticleData> getAllByStudyId(final Long studyId, final Pageable pageable,
                                             final String articleType) {
        final List<ArticleData> content = getContent(studyId, pageable, articleType);
        final int totalCount = getTotalCount(studyId, articleType);
    ...
    private int getTotalCount(final Long studyId, final String articleType) {
        final String sql = "SELECT count({}.id) FROM {} WHERE {}.study_id = :studyId";
        final Map<String, Long> param = Map.of("studyId", studyId);
        return namedParameterJdbcTemplate.queryForObject(sql.replaceAll("\\{\\}", articleType), param, (rs, rn) -> rs.getInt(1));
    }
}
