...
public class PostArticleDao {
    ...
    public static final RowMapper<PostArticleData> ROW_MAPPER = (rs, rn) -> {
        ...
        return new PostArticleData(id, memberData, title, content, createdDate, lastModifiedDate);
    };
    private final NamedParameterJdbcTemplate namedParameterJdbcTemplate;


    public PostArticleDao(final NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
    }

    public Optional<PostArticleData> getById(final Long communityId, ArticleType type) {
    ...
    public Page<PostArticleData> getAllByStudyId(final Long studyId, final Pageable pageable, ArticleType type) {
        final List<PostArticleData> content = getContent(studyId, pageable, type);
        final int totalCount = getTotalCount(studyId, type);
        return new PageImpl<>(content, pageable, totalCount);
    }

    private List<PostArticleData> getContent(final Long studyId, final Pageable pageable, ArticleType type) {
        final String sql = "SELECT {}.id as article_id, {}.title as article_title, {}.content as article_content, "
                + "{}.created_date as article_created_date, {}.last_modified_date as article_last_modified_date, "
                + "member.github_id, member.username, member.image_url, member.profile_url "
                + "FROM {} JOIN member ON {}.author_id = member.id "
