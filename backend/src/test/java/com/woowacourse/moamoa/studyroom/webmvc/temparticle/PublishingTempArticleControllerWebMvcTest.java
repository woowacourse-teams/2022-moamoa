...
class PublishingTempArticleControllerWebMvcTest extends WebMVCTest {

    private static final String URL = "/api/studies/{study-id}/{article-type}/draft-articles/{article-id}/publish";
    ...
    void unauthorizedByEmptyToken(final String type) throws Exception {
        mockMvc.perform(post(URL, 1L, type, 1L))
    ...
    void badRequestByInvalidIdFormat(String studyId, String articleId, String type) throws Exception {
        ...
        mockMvc.perform(post(URL, studyId, type, articleId)
                .header(HttpHeaders.AUTHORIZATION, token)
        )
