...
class TempCommunityArticleAcceptanceTest extends AcceptanceTest {
    ...
    void getTempArticles() {
        ...
        그린론이().로그인하고().스터디에(자바_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목1", "내용1"));
        그린론이().로그인하고().스터디에(자바_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목2", "내용2"));
        그린론이().로그인하고().스터디에(자바_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목3", "내용3"));
        final long 게시글4_ID = 그린론이().로그인하고().스터디에(자바_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목4", "내용4"));
        final long 게시글5_ID = 그린론이().로그인하고().스터디에(자바_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목5", "내용5"));
        final long 게시글6_ID = 그린론이().로그인하고().스터디에(리액트_스터디_ID).임시_게시글을().작성한다(new ArticleRequest("제목6", "내용6"));

        그린론이().로그인하고().스터디에(자바_스터디_ID).임시_공지사항을().작성한다(new ArticleRequest("공지1", "공지내용"));
        그린론이().로그인하고().스터디에(자바_스터디_ID).임시_공지사항을().작성한다(new ArticleRequest("공지2", "공지내용"));
    ...
    void publishTempArticle() {
        ...
        // act
        final long 게시글_ID = 그린론이().로그인하고().스터디에(스터디_ID).임시_게시글을().API_문서화를_하고(임시글_공개_문서(spec)).공개한다(임시_게시글_ID, articleRequest);
